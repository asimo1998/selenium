...
public class PageObjectBase {
    ...
    // Bach
    //===================================================
    protected boolean isSortDataAnphabetically(ILocator locator) {
        List<WebElement> element = getWebElements(locator);

        List<String> actualtResult = new ArrayList<String>();

        List<String> expectResult = new ArrayList<String>();

        for (WebElement el : element) {
            actualtResult.add(el.getText().toLowerCase());
            expectResult.add(el.getText().toLowerCase());
        }

        Collections.sort(expectResult);

        boolean checkAnphabet = actualtResult.equals(expectResult);
        System.out.println("return: " + checkAnphabet);

        return checkAnphabet;
    }

    protected String getDateTimeCreation(ILocator locator) {
        Sleep.refreshTime();
        WebElement element = getWebElement(locator);
        Sleep.refreshTime();

        String[] datetime = element.getText().split("\\s");
        //String datetime = element.getText();

        Sleep.refreshTime();
        String actualDatetime = datetime[0].replaceAll(",", " ").trim();

//        Sleep.refreshTime();
//        DateTimeFormatter myFormatObj = DateTimeFormatter.ofPattern("dd/MM/yyyy");
//        String actualDatetimeFormat = datetime.format(String.valueOf(myFormatObj)).trim();

//        System.out.println(actualDatetimeFormat);
    ...
    protected void importFile(String url) {
        Sleep.refreshTime();
        Robot robot = null;
        try {
            robot = new Robot();
        ...
        StringSelection urlCustom = new StringSelection(url);
        Toolkit.getDefaultToolkit().getSystemClipboard().setContents(urlCustom, null);
    ...
    protected String getText(ILocator locator) {
        WebElement element = getWebElement(locator);
